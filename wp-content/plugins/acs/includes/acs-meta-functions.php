 ...
 * acs_get_meta
 *
 * Returns an array of "ACS only" meta for the given post_id.
 *
 * @date	9/10/18
 * @since	5.8.0
 *
 ...
 * @return	array
 */
function acs_get_meta( $post_id = 0 ) {
	
	// Allow filter to short-circuit load_value logic.
	$null = apply_filters( "acs/pre_load_meta", null, $post_id );
	if( $null !== null ) {
		return ( $null === '__return_null' ) ? null : $null;
	}
	
	// Decode $post_id for $type and $id.
	extract( acs_decode_post_id($post_id) );
	...
	if( $type === 'option' ) {
		$allmeta = acs_get_option_meta( $id );
	} else {
	 ...
	 */
	return apply_filters( "acs/load_meta", $meta, $post_id );
}
 ...
 * @return	array
 */
function acs_get_option_meta( $prefix = '' ) {
 ...
 * @return	mixed
 */
function acs_get_metadata( $post_id = 0, $name = '', $hidden = false ) {
	
	// Allow filter to short-circuit logic.
	$null = apply_filters( "acs/pre_load_metadata", null, $post_id, $name, $hidden );
	if( $null !== null ) {
		return ( $null === '__return_null' ) ? null : $null;
	}
	...
	// Bail early if no $id (possible during new acs_form).
	if( !$id ) {
		return null;
	}
 ...
 */
function acs_update_metadata( $post_id = 0, $name = '', $value = '', $hidden = false ) {
	
	// Allow filter to short-circuit logic.
	$pre = apply_filters( "acs/pre_update_metadata", null, $post_id, $name, $value, $hidden );
	if( $pre !== null ) {
		return $pre;
	}
 ...
 * @return	bool
 */
function acs_delete_metadata( $post_id = 0, $name = '', $hidden = false ) {
	
	// Allow filter to short-circuit logic.
	$pre = apply_filters( "acs/pre_delete_metadata", null, $post_id, $name, $hidden );
	if( $pre !== null ) {
		return $pre;
	}
 ...
 * @return	void
 */
function acs_copy_metadata( $from_post_id = 0, $to_post_id = 0 ) {
	
	// Get all postmeta.
	$meta = acs_get_meta( $from_post_id );
	...
	if( $meta ) {
		...
		foreach( $meta as $name => $value ) {
			acs_update_metadata( $to_post_id, $name, $value );
		}
	}
}
 ...
 * @return	void
 */
function acs_copy_postmeta( $from_post_id = 0, $to_post_id = 0 ) {
	return acs_copy_metadata( $from_post_id, $to_post_id );
}
 ...
 */
function acs_get_meta_field( $key = 0, $post_id = 0 ) {
	
	// Try reference.
	$field_key = acs_get_reference( $key, $post_id );
	
	if( $field_key ) {
		$field = acs_get_field( $field_key );
		if( $field ) {
			$field['name'] = $key;
			return $field;
		}
 ...
 * @return	mixed
 */
function acs_get_metaref( $post_id = 0, $type = 'fields', $name = '' ) {
	
	// Load existing meta.
	$meta = acs_get_metadata( $post_id, "_acs_$type" );
 ...
 * @return	(int|bool) Meta ID if the key didn't exist, true on successful update, false on failure.
 */
function acs_update_metaref( $post_id = 0, $type = 'fields', $references = array() ) {
	
	// Get current references.
	$current = acs_get_metaref( $post_id, $type );
	...
	// Update metadata.
	return acs_update_metadata( $post_id, "_acs_$type", $references );
} 
